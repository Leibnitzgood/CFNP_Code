...
class CorrEncoder(nn.Module):
    ...
    def forward(self, ZR, ZM, XR, XM):
        ...
        HR = self.ref_enc(ZR)
        HM = self.data_enc(ZM)
        A = bipartite(HM, HR, self.pairwise_g)
...
class Decoder(nn.Module):
    ...
    def predict(self, zs, sRs, HM, sample=True):
        ...
        py = Normal(mean_y, logstd_y)
        if sample:
            samples = [py.sample() for _ in range(40)]
            out = torch.mean(torch.stack(samples), dim=0)
        else:
            out = mean_y
        std_y = torch.exp(logstd_y)

        return out, mean_y, logstd_y, attens, std_y

def create_corr_encoder(device):
    return CorrEncoder(
        in_data_dim=60,
        in_data_det_dim=60,
        in_ref_dim=60,
